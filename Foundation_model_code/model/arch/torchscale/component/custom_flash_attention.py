...
if torch.cuda.is_available():
    try:
        if torch.cuda.get_device_capability()[0] > 7:
            ...
            def flash_attn_varlen_func(q, k, v, dropout=0.0, bias=None, key_padding_mask=None, softmax_scale=None,
                                       is_causal=False):
                # q, k, v: [b, s, h, d]
                assert bias is None
            ...
            class FlashAttnFunc(torch.autograd.Function):
                @staticmethod
                # type: ignore
                def forward(ctx, q, k, v, dropout=0.0, bias=None, key_padding_mask=None, softmax_scale=None,
                            is_causal=False):
                    if is_causal:
                        assert bias is None
