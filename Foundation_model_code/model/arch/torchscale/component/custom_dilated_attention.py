...
class DilatedAttention(MultiheadAttention):
    ...
    def forward(
        ...
        for sl, dr in zip(self.args.segment_length, self.args.dilated_ratio):
            ...
            qi = self.gathering(q, dr, sl, is_causal=is_causal, offset=offset, is_kv=False,
                                seq_parall=self.args.seq_parallel)

            # added by Hanwen, split key_padding_mask
            if key_padding_mask is not None:
                key_padding_mask_i = self.gathering(key_padding_mask, dr, sl, is_causal=is_causal, offset=0, is_kv=True,
                                                    seq_parall=self.args.seq_parallel)
            else:
                key_padding_mask_i = None

            out, lse = self.attention_ops(qi, ki, vi, key_padding_mask=key_padding_mask_i, attn_mask=attn_mask,
                                          rel_pos=rel_pos, is_causal=is_causal)
