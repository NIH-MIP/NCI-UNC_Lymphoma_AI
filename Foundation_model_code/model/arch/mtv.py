...
class CrossViewAttentionEncoderBlock(nn.Module):
    ...
    def get_stochastic_depth_mask(self, lyr: int, num_layers: int, x: np.ndarray,
    ...
    def _apply_self_attentions(self, tokens: List[np.ndarray], cur_layer: int,
                               deterministic: bool) -> List[np.ndarray]:
    ...
    def _apply_mlp(self, tokens: List[np.ndarray], cur_layer: int,
                   deterministic: bool) -> List[np.ndarray]:
        """Applies MLP block."""
        for view_idx, x in enumerate(tokens):
            if cur_layer >= self.view_configs[view_idx]['num_layers']:
    ...
    @nn.compact
    def __call__(self, tokens: List[np.ndarray], cur_layer: int,
                 deterministic: bool) -> List[np.ndarray]:
...
class MultiviewEncoder(nn.Module):
    ...
    def _split_tokens_and_bottleneck(
            self, tokens: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _add_posembed(self, tokens: Sequence[np.ndarray]) -> List[np.ndarray]:
                ...
                if lyr in fusion_layers:
                    if xs[view_idx].shape[-1] != bottleneck.shape[-1]:
                        bottleneck = nn.Dense(
                            ...
                            bottleneck)
                    xs[view_idx] = np.concatenate([xs[view_idx], bottleneck], axis=1)
    ...
    def __call__(self,
                 ...
                 bottleneck: Union[np.ndarray, None],
                 train: bool = False) -> List[np.ndarray]:
...
class MTV(nn.Module):
    ...
    def _add_cls_token(self, x: np.ndarray, name: str) -> np.ndarray:
        """Prepends CLS token.
        Args:
        ...
        if self.classifier == 'token':
            ...
            cls = np.tile(cls, [bs, 1, 1])
            x = np.concatenate([cls, x], axis=1)
        return x

    def _add_cls_tokens_all_frames(self, x: np.ndarray,
                                   name: str) -> np.ndarray:
        ...
        if self.classifier == 'token':
            ...
            cls = np.tile(cls, [bs, 1, 1, 1])
            x = np.concatenate([cls, x], axis=2)
        return x

    def _add_cls_tokens_for_all_views(
            self, tokens: Sequence[np.ndarray]) -> List[np.ndarray]:
    ...
    def _extract_encoder_output(self,
        ...
        if self.classifier in ['token', '0']:
            ...
            fn = {'gap': np.mean, 'gmp': np.max, 'gsp': np.sum}[self.classifier]
            x = fn(x, axis=list(range(axis, x.ndim - 1)))
        return x

    def _tokenize(self, x: np.ndarray) -> List[np.ndarray]:
    ...
    def _merge_views_along_time_axis(self, tokens: Sequence[np.ndarray],
        ...
        return np.concatenate(projected_tokens, axis=1)

    def _merge_views_along_channel_axis(
            self, tokens: Sequence[np.ndarray]) -> np.ndarray:
        ...
        for idx, x in enumerate(tokens):
            ...
            if self.keep_spatiotemporal_features:
                xs.append(np.tile(x, (1, max_temporal_dim // x.shape[1], 1, 1)))
            else:
                x = self._extract_encoder_output(x, axis=2)
                xs.append(np.tile(x, (1, max_temporal_dim // x.shape[1], 1)))
        return np.concatenate(xs, axis=-1)

    def _global_encode(self, tokens: Sequence[np.ndarray],
    ...
    def _check_config(self, x: np.ndarray):
        ...
        if (self.cross_view_fusion is not None and
            ...
            bottleneck = self.param(
                ...
                (1, tokens[0].shape[1], self.cross_view_fusion.bottleneck_tokens,
                 channels), self.dtype)
            bottleneck = np.tile(bottleneck, [x.shape[0], 1, 1, 1])
            tokens = jax.vmap(
...
class MTVClassificationModel(vivit_model.ViViTClassificationModel):
    ...
    def build_flax_model(self) -> nn.Module:
        model_dtype = getattr(np, self.config.get('model_dtype_str', 'float32'))
        return MTV(
