...
from model.arch.pos_embed import get_2d_sincos_pos_embed
from model.arch.torchscale.model.LongNet import make_longnet_from_name
...
def create_model(pretrained: str, model_arch: str, in_chans: int,
                 local_dir: str = os.path.join(os.path.expanduser("~"), ".cache/"), **kwargs):
    ...
    if pretrained.startswith("hf_hub:"):
        ...
        huggingface_hub.hf_hub_download(hub_name, filename="slide_encoder.pth", local_dir=local_dir,
                                        force_download=True)
    ...
    if os.path.exists(local_path):
        ...
        print(
            "\033[93m Pretrained weights not found at {}. Randomly initialized the model! \033[00m".format(local_path))
